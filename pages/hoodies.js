...
const Hoodies = ({ products }) => {
  return Object.keys(products).length != 0 ? (
    <div>
      <Head>
        <title>Hoodies | Developer Wear</title>
        <meta
          name="description"
                ...
                >
                  ...
                  <div className="mt-4">
                    ...
                    {products[item].color.includes("white") && (
                      <button
                        className={`border-2 border-white m-1 bg-white rounded-full w-6 h-6 focus:outline-none`}
                      ></button>
                    )}
                  </div>
                </Link>
    ...
    <div className="container text-center my-3">
      No products available of this category!
    </div>
  );
};
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: ["hoodies", "Hoodies"] });
  let tshirts = {};
